...
function populateTable(persons) {
  ...
  persons.forEach(person => {
    ...
    let partnerLinks = person.spouses ? person.spouses.map(partner => 
      `<a href="${SITE_BASE_URL}/person/${partner.id}" class="link">${partner.GIVN} ${partner.SURN}</a>`
    ).join('<br>') : ''; // Join with a line break

    // Build the row string without updating the DOM yet
    rows += `<tr>
               <td><a href="${SITE_BASE_URL}/person/${person.id}" class="link">${person.GIVN}</a></td>
               <td><a href="${SITE_BASE_URL}/person/${person.id}" class="link">${person.SURN}</a></td>
               <td>${partnerLinks}</td>
